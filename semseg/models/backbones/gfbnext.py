...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, H, W) -> Tensor:
        ...
        k, v = self.kv(x).reshape(B, -1, 2, self.head, C // self.head).permute(2, 0, 3, 1, 4)                # B N 2C -> B, N, 2, head, C//head -> 2, B, head, N, C/head

        attn = (q @ k.transpose(-2, -1)) * self.scale     # (B, head, N, C/head) * (B, head, C/head, N
    ...
    def __init__(self, model_name: str = 'B0', modals: list = ['rgb', 'depth', 'event', 'lidar']):
        ...
        if self.num_modals > 0:
            ...
            self.EARMs = nn.ModuleList([
                EARM(dim=embed_dims[0], num_heads=num_heads[0], sr_ratio=8),
                EARM(dim=embed_dims[1], num_heads=num_heads[1], sr_ratio=4),
                EARM(dim=embed_dims[2], num_heads=num_heads[2], sr_ratio=2),
                EARM(dim=embed_dims[3], num_heads=num_heads[3], sr_ratio=1)])
            self.CFMs = nn.ModuleList([
                CFM(dim=embed_dims[0], ratio=2, num_heads=num_heads[0], kernel=7, norm_layer=nn.BatchNorm2d),
                CFM(dim=embed_dims[1], ratio=2, num_heads=num_heads[1], kernel=7, norm_layer=nn.BatchNorm2d),
                CFM(dim=embed_dims[2], ratio=1, num_heads=num_heads[2], kernel=7, norm_layer=nn.BatchNorm2d),
                CFM(dim=embed_dims[3], ratio=1, num_heads=num_heads[3], kernel=5, norm_layer=nn.BatchNorm2d)])
