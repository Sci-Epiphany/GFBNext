...
class MCubeSBGM(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        _mask_b = os.path.join(*[self.root, 'Edge', item_name + '.png'])              # Boundary GroundTruth
        ...
        _img, _target, _aolp, _dolp, _nir, _nir_mask, _mask , _mask_b= _img[:, self._left_offset:], _target[:,
                                                                                           self._left_offset:], \
            _aolp[:, self._left_offset:], _dolp[:, self._left_offset:], \
            _nir[:, self._left_offset:], _nir_mask[:, self._left_offset:], _mask[:, self._left_offset:], _mask_b[:, self._left_offset:]
        sample = {'image': _img, 'label': _target, 'aolp': _aolp, 'dolp': _dolp, 'nir': _nir, 'nir_mask': _nir_mask,
                  'u_map': self.u_map, 'v_map': self.v_map, 'mask': _mask, 'mask_b': _mask_b}
        ...
        label = [sample['label'].long(), sample['mask_b'].long()]
...
class RandomScaleCrop(object):
    ...
    def __call__(self, sample):
        ...
        mask_b = cv2.resize(mask_b, (ow, oh), interpolation=cv2.INTER_NEAREST)
        ...
        if short_size < self.crop_size:
            ...
            mask_[:oh, :ow] = mask
            mask = mask_
            mask_bb = np.full((oh + padh, ow + padw), self.fill)
            mask_bb[:oh, :ow] = mask_b
            mask_b = mask_bb
        ...
        mask_b = mask_b[y1:y1 + self.crop_size, x1:x1 + self.crop_size]
...
if __name__ == '__main__':
    ...
    trainset = MCubeSBGM(transform=traintransform, split='val')
