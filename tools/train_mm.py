...
from semseg.losses import get_loss, get_uw_loss, get_aux_loss
...
def main(cfg, gpu, save_dir):
    ...
    # add uw loss variable
    log_var_a = torch.ones((1,), requires_grad=True)
    log_var_b = torch.ones((1,), requires_grad=True)
    log_vars = [log_var_a, log_var_b]
    if model_cfg['NAME'] == 'GFBNEXT':
        loss_fn = get_aux_loss(loss_cfg['NAME'], trainset.ignore_label, None, log_vars=log_vars, device=device)
    elif model_cfg['NAME'] == 'GFBNBGM':
        loss_fn = get_uw_loss(loss_cfg['NAME'], trainset.ignore_label, None, log_vars=log_vars, device=device)
    else:
        loss_fn = get_loss(loss_cfg['NAME'], trainset.ignore_label, None)


    start_epoch = 0
    optimizer = get_optimizer(model, optim_cfg['NAME'], lr, optim_cfg['WEIGHT_DECAY'], log_vars, bgm)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for iter, (sample, lbl) in pbar:
            ...
            if bgm == 0:
                ...
                lbl = [lbl.to(device) for lbl in lbl]
            
            with autocast(enabled=train_cfg['AMP']):
                # The losses are divided into two cases: bounded guidance and boundless guidance
                if bgm == 0:
                    logits = model(sample)
                    loss = loss_fn(logits, lbl)

                elif model_cfg['NAME'] == 'GFBNEXT':
                    logits, logits_aux, boundary = model(sample)
                    loss = loss_fn(logits, logits_aux, boundary, lbl[0], lbl[1])
                else:
                    logits, boundary = model(sample)
                    loss = loss_fn(logits, boundary, lbl[0], lbl[1])
        ...
        if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
            ...
            if bgm == 1:
                log_var_a_s = log_var_a.cpu().detach().numpy()
                log_var_b_s = log_var_b.cpu().detach().numpy()
                writer.add_scalar('train/log_var_a', log_var_a_s, epoch)
                writer.add_scalar('train/log_var_b', log_var_b_s, epoch)

        # use the boundary module, save the log_var

        torch.cuda.empty_cache()

        if ((epoch+1) % train_cfg['EVAL_INTERVAL'] == 0 and (epoch+1)>train_cfg['EVAL_START']) or (epoch+1) == epochs:
            if (train_cfg['DDP'] and torch.distributed.get_rank() == 0) or (not train_cfg['DDP']):
                acc, macc, _, _, ious, miou = evaluate(model, valloader, device, bgm)
